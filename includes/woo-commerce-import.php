<?php /*
    This file is a modified version of the Woo Product Importer.
...
function jem_sf_woocommerce_enabled()
{
    return $GLOBALS['woocommerce'] != null;
}

function jem_sf_import_woocommerce_action()
{       
    $pageNumber = $_GET['pageNumber'];
    $storeId = $_GET['storeId'];
    $result = false;
    $result = jem_sf_import_woocomerce($storeId, $pageNumber);    
    die(json_encode($result));
}

function jem_sf_import_woocomerce($storeId, $pageNumber) 
{    
    $importedProducts = 0;
    $result = array();
    if ($pageNumber > 25)
    {
        ...
        $result['errorMsg'] = 'Can only import up to 30 pages of your store';
        return $result;
    }
    ...
    {
        jem_sf_import_woocommerce_product($product, $response->Data->ImportCategory, true);
        $importedProducts++;
    ...
    $result['hasMore'] = $pageNumber < 26 && $response->Data->TotalPages > $pageNumber;    
    $result['errorMsg'] = false;
    return $result;                   
}

function jem_sf_import_woocommerce_product($ad, $storeName, $partialUpdate) {
    ...
    $row = array(
        ...
        'post_title' => html_entity_decode($ad->Name),
        'post_content' => html_entity_decode($ad->ShortDescription),        
        '_sku' => $ad->MerchantId . '-' . $sku,
        '_button_text' => 'Learn More',
        '_product_url' => $ad->Url,
        'product_cat_by_name' => $storeName,
        'product_image_by_url' => $ad->BigImageUrl
    );   
    
    if ($ad->Price < $ad->ListPrice)
    {
        $row['_regular_price'] = $ad->ListPrice/100;
        $row['_sale_price'] = $ad->Price/100;
    }
    else
    {
        $row['_regular_price'] = $ad->Price/100;
    }
    ...
    if(strlen($new_post['post_title']) > 0 || $existing_product !== null) {
        ...
        if($existing_product !== null) {
            ...
            if ($partialUpdate)
            {
                $new_post_id = $existing_product->ID;                
            } else {
                $new_post_id = wp_update_post($new_post);
            }
                        
            //sometimes we only want to update the prices and the URL
            if ($partialUpdate){
                $update_keys = array('_sale_price', '_regular_price', '_price', '_product_url');
                $scratch_post_meta = array();
                foreach ($update_keys as $update_meta_key){
                    if (array_key_exists($update_meta_key, $new_post_meta)) {
                        $scratch_post_meta[$update_meta_key] = $new_post_meta[$update_meta_key];
                    } else {
                        $scratch_post_meta[$update_meta_key] = null;
                    }
                }
                $new_post_meta = $scratch_post_meta;
            }
        ...
        if(is_wp_error($new_post_id)) {
            ...
            {             
                ...
                foreach($new_post_image_urls as $image_index => $image_url) {
                    ...
                    //getting the extension right doesn't actually matter for viewing in browser,
                    //depends on mime type.
                    if(!in_array($image_ext, $allowed_extensions)) {
                        $pathinfo['basename'] = $pathinfo['basename'] . '.jpg';
                        $image_ext = 'jpg';
                    }
...
function jem_sf_get_image_extension($contentType) {
}
